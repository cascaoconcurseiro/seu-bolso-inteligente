...
- [x] **[SSOT-03 parcial]** Das 14 funções de "balance", só 3 eram realmente órfãs (sem trigger, sem RPC do frontend, sem cron job): `calculate_account_balance`, `recalculate_all_account_balances`, `sync_account_balance` — dropadas. Verificado ao vivo: as 3 não existem mais.
  ...
  - Permanecem para auditoria individual: `recalculate_account_balance`, `calculate_single_account_balance`, `recalculate_all_balances`, `get_account_balance_at_date`, `create_account_with_balance`, `get_trip_participant_balances`, `calculate_balance_between_users`, `calculate_member_balance` e `settle_balance_between_users`.
