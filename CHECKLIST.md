...
- [x] **[CI-ROOT-CAUSE]** Causa raiz **confirmada** (16/07, texto literal do GitHub na página do run #150): **"The job was not started because your account is locked due to a billing issue."** — bloqueio de nível de conta pessoal do GitHub (não do repositório, não das actions permissions — "Allow all actions" já está selecionado). Trava todos os 6 jobs em ~5s sem executar nenhum step, independente do código. Explica os runs #120–#149 (12/07–16/07) e por que os fixes de código (Prettier, RPCs v2, etc.) nunca vão aparecer como verdes em CI enquanto isso não for resolvido. **Ação exclusiva do usuário** (envolve pagamento/conta — fora do escopo do assistente): resolver a pendência em `github.com/settings/billing` (cartão expirado/fatura em aberto), depois re-rodar o workflow.
