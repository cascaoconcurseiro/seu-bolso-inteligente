...
async function cleanupAll() {
  ...
  let r = await api("GET", `transactions?user_id=eq.${UID}&description=ilike.${PRE}*&select=id&limit=100`);
  ...
  for (const tbl of ["budgets", "goals", "notifications", "trips"]) {
    ...
    if (r.ok && Array.isArray(r.data)) {
      const testItems = r.data.filter(item =>
        (item.name || item.title || "").startsWith(PRE)
      );
...
async function t1() {
  ...
  const tables = [
    "transactions", "transaction_splits", "accounts", "categories", "budgets",
    "goals", "notifications", "trips", "family_members", "credit_card_invoices",
    "settlement_reversals", "transaction_auto_share_rules", "error_logs",
    "audit_log", "assets",
  ];
...
async function t3() {
  ...
  if (!accId) { log("Transações", false, "Sem conta corrente"); return; }

  const base = { user_id: UID, creator_user_id: UID, date: "2026-06-30", competence_date: "2026-06-01", is_shared: false, is_recurring: false, is_installment: false };

  // Expense
  let r = await api("POST", "transactions?select=id,amount,description,type", { ...base, amount: 75, description: PRE + "Almoço", type: "EXPENSE", domain: "PERSONAL", account_id: accId, category_id: catId });
  log("Criar DESPESA", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  if (r.ok && r.data?.[0]?.id) created.tx.push(r.data[0].id);

  // Income
  r = await api("POST", "transactions?select=id,amount,description,type", { ...base, amount: 5000, description: PRE + "Salário", type: "INCOME", domain: "PERSONAL", account_id: accId, category_id: incId });
  log("Criar RECEITA", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  if (r.ok && r.data?.[0]?.id) created.tx.push(r.data[0].id);

  // Transfer
  if (savId) {
    r = await api("POST", "transactions?select=id,amount,description,type", { ...base, amount: 200, description: PRE + "Transferência", type: "TRANSFER", domain: "PERSONAL", account_id: accId, destination_account_id: savId, destination_amount: 200 });
    log("Criar TRANSFERÊNCIA", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
    if (r.ok && r.data?.[0]?.id) created.tx.push(r.data[0].id);
  }
...
async function t4() {
  ...
  if (!member) { log("Compartilhadas", false, "Sem family_member"); return; }
  let ar = await api("GET", `accounts?user_id=eq.${UID}&type=eq.CHECKING&limit=1`);
  const accId = ar.ok && ar.data?.[0]?.id;
  let cr = await api("GET", `categories?type=eq.expense&limit=1`);
  const catId = cr.ok && cr.data?.[0]?.id;
  ...
  let r = await api("POST", "transactions?select=*", {
    user_id: UID, creator_user_id: UID, amount: 250, description: PRE + "Jantar Compartilhado",
    date: "2026-06-30", competence_date: "2026-06-01", type: "EXPENSE", domain: "SHARED",
    account_id: accId, category_id: catId, is_shared: true, payer_id: member.id,
  ...
  if (r.ok && r.data?.[0]?.id) {
    ...
    const splits = [{ transaction_id: r.data[0].id, member_id: member.id, user_id: UID, percentage: 50, amount: 125, name: "Você", is_settled: false }];
    if (other) splits.push({ transaction_id: r.data[0].id, member_id: other.id, user_id: other.linked_user_id, percentage: 50, amount: 125, name: "Outro", is_settled: false });
    ...
    if (other) {
      let st = await api("POST", "transactions?select=id", {
        user_id: other.linked_user_id || UID, creator_user_id: other.linked_user_id || UID,
        amount: 125, description: PRE + "Acerto Jantar", date: "2026-06-30",
        competence_date: "2026-06-01", type: "EXPENSE", domain: "SHARED",
        account_id: accId, is_shared: true, is_recurring: false, is_installment: false,
        related_member_id: member.id, is_settled: true,
      });
      log("Criar acerto (settlement)", st.ok, st.ok ? `id=${st.data[0]?.id?.slice(0,8)}` : JSON.stringify(st.data).slice(0,80));
...
async function t5() {
  ...
  if (!card) { log("Parcelamento", false, "Sem cartão"); return; }
  const seriesId = crypto.randomUUID();
  let allOk = true;
  for (let i = 0; i < 3; i++) {
    const mon = 6 + i;
    const r = await api("POST", "transactions?select=id", {
      user_id: UID, creator_user_id: UID, amount: 100, description: `${PRE} Parcela ${i + 1}/3`,
      date: `2026-${String(mon).padStart(2, "0")}-30`, competence_date: `2026-${String(mon).padStart(2, "0")}-01`,
      type: "EXPENSE", domain: "PERSONAL", account_id: card.id,
      is_installment: true, total_installments: 3, current_installment: i + 1,
      series_id: seriesId, is_shared: false, is_recurring: false,
    });
    if (r.ok && r.data?.[0]?.id) created.tx.push(r.data[0].id); else allOk = false;
  }
...
async function t6() {
  ...
  if (!cat) { log("Orçamento", false, "Sem categoria"); return; }
  let r = await api("POST", "budgets?select=*", {
    user_id: UID, category_id: cat.id, amount: 1000, period: "MONTHLY",
    start_date: "2026-06-01", end_date: "2026-12-31", is_active: true, notification_threshold: 80,
  });
  log("Criar orçamento", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  if (r.ok && r.data?.[0]?.id) {
...
async function t7() {
  ...
  let r = await api("POST", "goals?select=*", {
    user_id: UID, name: PRE + "Viagem Praia", target_amount: 5000, current_amount: 0,
    start_date: "2026-06-30", target_date: "2026-12-31", type: "GOAL", status: "ACTIVE", currency: "BRL",
  });
  log("Criar meta", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  if (r.ok && r.data?.[0]?.id) {
...
async function t8() {
  ...
  let r = await api("POST", "notifications?select=*", {
    user_id: UID, type: "SYSTEM", title: PRE + "Notificação Teste",
    message: "Teste automatizado.", icon: "🔔", priority: "NORMAL", is_read: false,
  });
  log("Criar notificação", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  if (r.ok && r.data?.[0]?.id) {
...
async function t9() {
  ...
  let ar = await api("GET", `accounts?user_id=eq.${UID}&type=eq.CREDIT_CARD&limit=1`);
  const card = ar.ok && ar.data?.[0];
  if (!card) { log("Cartão", false, "Sem cartão"); return; }
  ...
  let pr = await api("POST", "rpc/process_credit_card_invoice", { p_account_id: card.id, p_reference_date: "2026-07-01" });
...
async function t10() {
  ...
  let r = await api("POST", "trips?select=*", {
    user_id: UID, name: PRE + "Rio de Janeiro", destination: "Rio de Janeiro",
    start_date: "2026-08-01", end_date: "2026-08-10", budget: 3000, currency: "BRL", status: "PLANNING",
  });
  log("Criar viagem", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  ...
  if (accId) {
    let tx = await api("POST", "transactions?select=id", {
      user_id: UID, creator_user_id: UID, amount: 450, description: PRE + "Passagem RJ",
      date: "2026-06-30", competence_date: "2026-06-01", type: "EXPENSE", domain: "TRAVEL",
      trip_id: r.data[0].id, account_id: accId, is_shared: false, is_recurring: false, is_installment: false,
    });
    log("Despesa na viagem", tx.ok, tx.ok ? `id=${tx.data[0]?.id?.slice(0,8)}` : JSON.stringify(tx.data).slice(0,80));
...
async function t11() {
  ...
  if (created.tx.length === 0) { log("Edição/Exclusão", false, "Sem transações"); return; }
  const txId = created.tx[0];
  let ed = await api("PATCH", `transactions?id=eq.${txId}`, { description: PRE + "EDITADO", amount: 888 });
  log("Editar transação", ed.ok);
  let sd = await api("PATCH", `transactions?id=eq.${txId}`, { deleted_at: new Date().toISOString() });
...
async function t12() {
  ...
  const rpcs = [
    ["create_transaction_with_splits", { p_transaction: { amount: 1, description: "X", date: "2026-06-30", competence_date: "2026-06-01", type: "EXPENSE", domain: "PERSONAL", is_shared: false, is_recurring: false, is_installment: false }, p_splits: [] }],
    ...
    ["process_credit_card_invoice", { p_account_id: "00000000-0000-0000-0000-000000000000", p_reference_date: "2026-07-01" }],
    ...
    ["settle_transaction_split", { p_split_id: "00000000-0000-0000-0000-000000000000", p_settled_by: UID }],
...
async function t13() {
  ...
  let ar = await api("GET", `accounts?user_id=eq.${UID}&type=eq.CHECKING&limit=1`);
  const accId = ar.ok && ar.data?.[0]?.id;
  if (!accId) { log("Recorrência", false, "Sem conta"); return; }
  let r = await api("POST", "transactions?select=id", {
    user_id: UID, creator_user_id: UID, amount: 39.90, description: PRE + "Streaming Mensal",
    date: "2026-06-30", competence_date: "2026-06-01", type: "EXPENSE", domain: "PERSONAL",
    account_id: accId, is_recurring: true, recurrence_pattern: "MONTHLY", recurrence_day: 30,
    is_shared: false, is_installment: false,
  });
  log("Criar transação recorrente", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  if (r.ok && r.data?.[0]?.id) created.tx.push(r.data[0].id);
}
...
async function t14() {
  ...
  let inv = await api("GET", `family_invitations?email=eq.wesley.diaslima@gmail.com&status=eq.PENDING&limit=5`);
...
async function t15() {
  ...
  if (!fm.ok || !fm.data?.[0]) { log("Auto-share", false, "Sem family_member"); return; }
  let r = await api("POST", "transaction_auto_share_rules?select=*", {
    user_id: UID, member_id: fm.data[0].id, description_pattern: PRE, split_ratio: 0.5, is_active: true,
  });
  log("Criar regra auto-share", r.ok, r.ok ? `id=${r.data[0]?.id?.slice(0,8)}` : JSON.stringify(r.data).slice(0,80));
  if (r.ok && r.data?.[0]?.id) await api("DELETE", `transaction_auto_share_rules?id=eq.${r.data[0].id}`);
}
...
async function finalCleanup() {
  ...
  let del = 0;
  for (const id of created.tx) { if ((await api("DELETE", `transactions?id=eq.${id}`)).ok) del++; }
