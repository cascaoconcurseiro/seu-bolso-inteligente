...
async function main() {
  ...
  try {
    ...
    r = await c.query(
      "SELECT enum_range(NULL::notification_type) as vals"
    );
    ...
    r.rows.forEach(rr => {
      ...
      if (match) console.log(rr.proname + " references financial_entities (" + match.length + " times)");
    });
    ...
    console.log("\nFinancial tables:", r.rows.map(rr => rr.schemaname + "." + rr.tablename).join(", ") || "NONE");
