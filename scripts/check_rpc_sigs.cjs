...
async function main() {
  ...
  try {
    ...
    for (const fn of fns) {
      let r = await c.query(
        "SELECT pg_get_function_arguments(oid) as a, pg_get_function_result(oid) as r FROM pg_proc WHERE proname=$1",
        [fn]
      );
      if (r.rows.length > 0) {
    ...
    try {
      let r = await c.query(
        "INSERT INTO transaction_splits (transaction_id, member_id, user_id, percentage, amount, name, is_settled) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id",
        [
    ...
    let r = await c.query(
      "SELECT conname, pg_get_constraintdef(oid) FROM pg_constraint WHERE conrelid='transaction_auto_share_rules'::regclass"
    );
