          ...
          {members.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {activeTab !== 'TRAVEL' ? members.filter(m => m.linked_user_id !== user?.id).map(m => {
                ...
                return <SharedExpenseCard key={m.id} member={m} items={items} netAmount={getTotals(items)['BRL']?.net || 0} currency="BRL" isHistory={activeTab === "HISTORY"} currentUserId={user?.id} formatCurrency={formatCurrency} onSettle={(id, type, amt) => { setSelectedMember(id); setSettleType(type); const pending = getFilteredInvoice(id).filter(i => !i.isPaid); setSelectedItems(pending.map(i => i.id)); setSettleAmount(Math.abs(amt).toFixed(2).replace(".", ",")); setShowSettleDialog(true); }} onUndo={(i) => setUndoConfirm({ isOpen: true, item: i })} onDelete={(i) => setDeleteConfirm({ isOpen: true, item: i })} onConfirmReceipt={(i) => setConfirmReceiptDialog({ isOpen: true, items: [i] })} onRejectSettlement={(i) => setRejectDialog({ isOpen: true, item: i, reason: "" })} />;
              }) : trips.filter(t => members.some(m => getFilteredInvoice(m.id).some(i => i.tripId === t.id))).map(t => <SharedTripCard key={t.id} trip={t} members={members} getFilteredInvoice={getFilteredInvoice} getTotals={getTotals} formatCurrency={formatCurrency} user={user} onSettle={(id, type, amt) => { setSelectedMember(id); setSettleType(type); const pending = getFilteredInvoice(id).filter(i => !i.isPaid); setSelectedItems(pending.map(i => i.id)); setSettleAmount(Math.abs(amt).toFixed(2).replace(".", ",")); setShowSettleDialog(true); }} onUndo={(i) => setUndoConfirm({ isOpen: true, item: i })} onDelete={(i) => setDeleteConfirm({ isOpen: true, item: i })} onDeleteSeries={(i) => setDeleteSeriesConfirm({ isOpen: true, item: i })} onConfirmReceipt={(i) => setConfirmReceiptDialog({ isOpen: true, items: [i] })} onAnticipate={(i) => setAnticipateDialog({ isOpen: true, seriesId: i.seriesId ?? null, currentInstallment: i.installmentNumber ?? 0, totalInstallments: i.totalInstallments ?? 0 })} />)}
