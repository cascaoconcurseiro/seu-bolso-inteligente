...
export class CategoryPredictionService {
  ...
  private static async checkUserLearning(
    ...
    try {
      ...
      const categoryObj = data.categories as any;
  ...
  private static async matchKeywords(
    ...
    try {
      ...
      for (const category of categories) {
        ...
        const foundKey = Object.keys(DEFAULT_KEYWORDS).find(key => {
          const keyLower = key.toLowerCase();
          return catNameLower === keyLower || 
                 catNameLower.includes(keyLower) || 
                 keyLower.includes(catNameLower);
        });
        
        const keywords = foundKey ? DEFAULT_KEYWORDS[foundKey] : null;
