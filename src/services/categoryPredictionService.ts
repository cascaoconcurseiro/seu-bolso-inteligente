      ...
      for (const row of data) {
        ...
        // D. Match de Tokens (Palavras)
          ...
          const allPatternWordsInInput = cleanPatternWords.every((pw) =>
            cleanInputWords.some(
              (iw) => iw === pw || (pw.length >= 4 && this.levenshteinDistance(pw, iw) <= 1)
            )
          );
  ...
  private static async matchKeywords(
    ...
    try {
      ...
      for (const category of candidates) {
        ...
        if (
          catNameLower === cleanConcept ||
          cleanConcept.includes(catNameLower) ||
          catNameLower.includes(cleanConcept)
        ) {
        ...
        const matchedSynonym = synonyms.find(
          (syn) => catNameLower === syn || catNameLower.includes(syn) || syn.includes(catNameLower)
        );
        if (matchedSynonym) {
      ...
      if (!matchedCategory) {
        for (const parent of parentCategories) {
          ...
          if (
            parentNameLower === cleanConcept ||
            synonyms.some((syn) => parentNameLower.includes(syn))
          ) {
  ...
  static async learnFromUser(
    ...
    try {
        ...
        if (competitors && competitors.length > 0) {
          for (const comp of competitors) {
            ...
            if (newConfidence <= 0.2) {
              ...
              await supabase.from("user_category_learning").delete().eq("id", comp.id);
            } else {
      ...
      if (existing) {
        ...
        await supabase.from("user_category_learning").insert({
          user_id: userId,
          description_pattern: cleanPattern,
          category_id: categoryId,
          confidence: wasCorrection ? 0.85 : 0.7,
          times_used: 1,
          last_used_at: new Date().toISOString(),
        });
      }
