...
export async function logSettlementOperation(
  ...
  try {
    logger.debug('Logging settlement operation', {
    ...
    logger.success('Operation logged successfully', { id: data.id });
    return data as AuditLogEntry;
  } catch (error) {
    logger.error('Unexpected error in auditLog', error);
    return null;
  }
}
...
export async function queryAuditLogs(
  ...
  try {
    logger.debug('Querying audit logs', filters);
    ...
    logger.debug('Found log entries', { count: data.length });
    return data as AuditLogEntry[];
  } catch (error) {
    logger.error('Unexpected error querying logs', error);
    return [];
  }
}
