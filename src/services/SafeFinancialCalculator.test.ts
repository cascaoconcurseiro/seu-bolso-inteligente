  ...
  describe("Propriedade: Invariante de Soma de Splits", () => {
    it("safeSum(splits) <= total + 1 centavo", () => {
      fc.assert(
        fc.property(
          fc.array(
            fc.integer({ min: 1, max: 1000000 }).map((x) => x / 100),
            {
              minLength: 1,
              maxLength: 20,
            }
    ...
    it("add(add(a, b), c) === add(a, add(b, c))", () => {
      fc.assert(
        fc.property(
          ...
          fc.double({ min: -1000, max: 1000, noNaN: true }),
          (a, b, c) => {
            const result1 = SafeFinancialCalculator.add(SafeFinancialCalculator.add(a, b), c);
            const result2 = SafeFinancialCalculator.add(a, SafeFinancialCalculator.add(b, c));
