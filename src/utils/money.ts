...
// Configura o Decimal.js para arredondamento padrão brasileiro (ROUND_HALF_UP).
// SafeFinancialCalculator também usa ROUND_HALF_UP — consistência entre os dois.
...
export const moneyUtils = {
  ...
  add: (...values: (string | number)[]): number => {
    const sum = values.reduce(
      (acc: Decimal, val) => acc.plus(moneyUtils.parseToDecimal(val)),
      new Decimal(0)
    );
