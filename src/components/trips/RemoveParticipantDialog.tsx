        ...
        <DialogContent className="max-w-md shadow-2xl w-full !bottom-0 !top-auto !translate-y-0 sm:!top-[50%] sm:!bottom-auto sm:!-translate-y-1/2 rounded-t-[2rem] sm:!rounded-4xl !rounded-b-none sm:!rounded-b-[2rem] p-0 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] sm:shadow-lg max-h-[90vh] flex flex-col border-b-0 sm:border-b bg-background pb-[env(safe-area-inset-bottom)] overflow-hidden pb-[env(safe-area-inset-bottom)]">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-sm text-muted-foreground font-medium">
              ...
              <span className="font-bold text-foreground">{participant.name}</span> porque ele(a) é
              o criador da viagem.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
    ...
    >
      <DialogContent className="w-full sm:max-w-md !bottom-0 !top-auto !translate-y-0 sm:!top-[50%] sm:!bottom-auto sm:!-translate-y-1/2 rounded-t-[2rem] sm:!rounded-lg !rounded-b-none sm:!rounded-b-lg p-0 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] sm:shadow-lg max-h-[90vh] flex flex-col border-b-0 sm:border-b bg-card overflow-hidden">
        ...
        <DialogHeader className="px-6 pt-2 pb-2 text-left shrink-0">
          ...
          <DialogDescription className="text-sm text-muted-foreground font-medium">
            Gerencie o encerramento da participação de{" "}
            <span className="font-bold text-foreground">{participant.name}</span> na viagem.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-4 my-4 max-h-[60vh] overflow-y-auto pr-2">
                  ...
                  <div>
                    ...
                    >
                      {isSettled
                        ...
                        : (owesGroup ? "-" : "+") +
                          moneyUtils.format(Math.abs(currentBalance), currency)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {isSettled ? (
                <div className="p-4 rounded-xl border border-success/10 bg-success/5 flex items-start gap-3">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Como este participante não possui pendências, você pode removê-lo diretamente. O
                    histórico de transações passadas dele continuará preservado na viagem.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  <p className="text-sm font-bold text-muted-foreground uppercase tracking-wider">
                    Como deseja lidar com as pendências?
                  </p>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="font-bold text-sm text-foreground">
                          Zerar e Remover (Recomendado)
                        </p>
                        <p className="text-sm text-muted-foreground mt-0.5">
                          Registrar um acerto completo de{" "}
                          {moneyUtils.format(Math.abs(currentBalance), currency)} antes de concluir
                          a remoção.
                        </p>
                      </div>
                    </div>
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="font-bold text-sm text-foreground">
                          Apenas Remover (Manter Histórico)
                        </p>
                        <p className="text-sm text-muted-foreground mt-0.5">
                          Cortar o acesso do participante na hora, mantendo o saldo pendente visível
            ...
            <div className="space-y-4 my-4 animate-fade-in">
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 flex gap-3">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Você está registrando um acerto no valor de{" "}
                    <span className="font-black text-foreground">
                      {moneyUtils.format(Math.abs(currentBalance), currency)}
                    </span>{" "}
                    para zerar as contas de{" "}
                    <span className="font-bold text-foreground">{participant.name}</span>. Por
                    favor, escolha a conta financeira associada.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {accounts.length === 0 ? (
                    <p className="text-sm text-destructive">
                      Nenhuma conta cadastrada para registrar a transação.
                    </p>
                  ) : (
