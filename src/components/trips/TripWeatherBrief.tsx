...
function WeatherIcon({ code, className = "h-5 w-5" }: { code: number; className?: string }) {
  ...
  if (isSnowCode(code)) return <Snowflake className={className} aria-hidden="true" />;
...
function buildPackingAdvice(days: DayWeather[]): string[] {
  ...
  const hasSnow = days.some((day) => isSnowCode(day.weatherCode));
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/60 px-4 py-4 sm:px-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 id="trip-weather-title" className="font-semibold">
              Clima da viagem
            </h2>
      ...
      {latitude === null || longitude === null ? (
        ...
        <div
          className="flex gap-3 overflow-hidden px-4 py-5 sm:px-5"
          role="status"
