    ...
    <AlertDialog open={isOpen} onOpenChange={handleOpenChange}>
      <AlertDialogContent className={`max-w-md border-border/50 ${sheetDialogCn}`}>
        ...
        {transaction.is_installment && (
          <div className="py-2 px-5 sm:px-6 space-y-3">
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  <Label
                    htmlFor="delete-none"
                    className="font-normal cursor-pointer text-muted-foreground hover:text-foreground transition-colors"
                  >
                    Apenas esta parcela ({transaction.current_installment}/
            ...
            <div className="p-3 rounded-xl border border-warning/20 bg-warning/5 text-warning text-sm font-semibold leading-relaxed animate-in fade-in duration-350">
              {cascadeType === "NONE" && (
                <p>
                  ⚠️ Esta ação removerá apenas a parcela atual de{" "}
                  {moneyUtils.format(itemAmount, txCurrency)} no extrato.
                </p>
              )}
              {cascadeType === "NEXT" && (
                <p>
                  ⚠️ Esta ação removerá {remainingInstallments} parcelas futuras, reduzindo{" "}
                  {moneyUtils.format(remainingValue, txCurrency)} do seu planejamento futuro.
                </p>
              )}
              {cascadeType === "ALL" && (
                <p>
                  ⚠️ Esta ação removerá TODAS as {totalInst} parcelas, limpando o total de{" "}
                  {moneyUtils.format(totalValue, txCurrency)} do seu extrato histórico e futuro.
                </p>
              )}
            </div>
          </div>
        ...
        <AlertDialogFooter className="px-5 pb-5 pt-2 sm:px-6 sm:pb-6">
          <AlertDialogCancel onClick={() => setCascadeType("NONE")} disabled={isDeleting}>
            Cancelar
          </AlertDialogCancel>
          <AlertDialogAction
