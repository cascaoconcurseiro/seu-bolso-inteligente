...
export function SharedSettleDialog({
  ...
  const internationalTrip =
    tripIds.length > 0 ? trips.find((t) => tripIds.includes(t.id) && t.currency !== "BRL") : null;
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-muted-foreground">
              {settlingMode === "SINGLE"
                ...
                : settleType === "PAY"
                  ? "Registre o pagamento da sua dívida total"
                  : "Registre o recebimento do valor total devido"}
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          {isInternationalSettlement && (
            <Alert className="border-accent/20 bg-accent/5 dark:bg-accent/10">
              ...
              <AlertDescription className="text-sm text-accent">
                Acerto de viagem internacional em{" "}
                <span className="font-semibold">{settlementCurrency}</span>.
              </AlertDescription>
            </Alert>
          )}
              ...
              return (
                <Alert className="border-primary/20 bg-primary/5">
                  ...
                  <AlertDescription className="text-sm text-primary/80 leading-relaxed">
                    <strong>Compensação Automática:</strong> Como você possui valores a pagar e a
                    receber com <strong>{member?.name}</strong>, o sistema abate os débitos e
                    créditos mútuos, resultando no valor líquido final.
                  </AlertDescription>
                </Alert>
              );
            }
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1">
              <ArrowRight
                className={cn(
                  "h-5 w-5",
                  settleType === "PAY" ? "text-destructive" : "text-success"
                )}
              />
              <p
                className={cn(
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                {settlingMode !== "SINGLE" && (
                  <Button
                    ...
                    size="sm"
                    onClick={onSelectAll}
                    className="text-xs h-7 px-2 text-primary shrink-0"
                  >
              ...
              {isItemsExpanded && (
                <div className="border border-border/60 rounded-2xl divide-y divide-border/40 overflow-hidden">
                  {pendingMemberItems
                    .filter((i) =>
                      settlingMode === "SINGLE" ? selectedItems.includes(i.id) : true
                    )
                    .map((item) => {
          ...
          <div className="grid gap-3">
            <div className="space-y-1.5">
              <Label className="text-sm font-semibold">
                Data do {settleType === "PAY" ? "Pagamento" : "Recebimento"}
              </Label>
              <Input
                type="date"
                value={settleDate}
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-semibold">
                Conta{" "}
                {isInternationalSettlement && (
                  <span className="text-muted-foreground font-normal">({settlementCurrency})</span>
                )}
              </Label>
