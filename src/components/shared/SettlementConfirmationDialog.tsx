...
export function SettlementConfirmationDialog({
  ...
  const total = currentItems.reduce(
    (sum, item) => SafeFinancialCalculator.add(sum, item.amount),
    0
  );
  ...
  const filteredAccounts = accounts.filter(
    (a) => a.type !== "CREDIT_CARD" && a.currency === currency
  );
  ...
  useEffect(() => {
    if (isOpen && filteredAccounts.length > 0) {
      ...
      if (!isAlreadyValid) {
        const defaultAcc =
          filteredAccounts.find((a) => a.type === "CHECKING") || filteredAccounts[0];
  ...
  const handleConfirm = async () => {
    if (filteredAccounts.length === 0) {
      toast.error(
        `Você não possui conta cadastrada na moeda da despesa (${currency}). Por favor, cadastre uma conta nessa moeda primeiro.`
      );
      return;
    }
    ...
    if (!selectedAccount || selectedAccount.currency.toUpperCase() !== currency.toUpperCase()) {
      toast.error(
        `A conta selecionada (${selectedAccount?.currency}) não é compatível com a moeda do acerto (${currency}).`
      );
      return;
    }
    ...
    if (isPayment) {
      if (selectedAccount.type !== "CREDIT_CARD") {
        if (selectedAccount.balance < total) {
          toast.error(
            `Saldo insuficiente na conta "${selectedAccount.name}". Saldo atual: ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: selectedAccount.currency }).format(selectedAccount.balance)}. Valor necessário: ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: selectedAccount.currency }).format(total)}`
          );
          return;
        }
      }
        ...
        <div className="p-6 pb-2 relative shrink-0">
          <DialogHeader>
            ...
            <DialogDescription className="text-base">
              Verifique os itens e escolha a conta de {isPayment ? "origem" : "destino"} para este{" "}
              {isPayment ? "pagamento" : "recebimento"}.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="px-6 pb-6 overflow-y-auto hide-scrollbar space-y-5 relative">
          ...
          <div
            className={cn(
              ...
              isPayment
                ? "bg-destructive/5 border-destructive/20 hover:bg-destructive/12"
                : "bg-success/5 border-success/20 hover:bg-success/12"
            )}
          >
                      ...
                      <div className="flex-1 min-w-0 pr-3">
                        ...
                        <p className="text-sm text-muted-foreground font-mono">
                          {(() => {
                            ...
                            return dateFns.isValid(dt) ? dateFns.format(dt, "dd MMM yyyy") : "-";
                          })()}{" "}
                          • {item.category}
                        </p>
                      </div>
          ...
          {filteredAccounts.length === 0 && (
            <div className="p-4 rounded-xl border border-destructive/20 bg-destructive/5 space-y-2 animate-in slide-in-from-top-2 duration-300">
              ...
              <p className="text-sm text-destructive/80 dark:text-destructive/80 leading-relaxed">
                Você não possui nenhuma conta de pagamento disponível na moeda desta despesa (
                {currency}).
                <strong>
                  {" "}
                  É necessário usar uma conta da mesma moeda para realizar este acerto.
                </strong>
              </p>
            </div>
          )}
