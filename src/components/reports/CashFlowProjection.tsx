...
export function CashFlowProjection({ transactions, currentBalance }: CashFlowProjectionProps) {
  const projection = useMemo(() => {
    ...
    const months = Array.from({ length: 4 }, (_, i) => {
      ...
      return { date: d, start: startOfMonth(d), end: endOfMonth(d), label: format(d, "MMMM yyyy", { locale: ptBR }) };
    });
    ...
    const recurring = transactions.filter(tx =>
      tx.series_id && !tx.deleted_at && (isFuture(new Date(tx.date)) || isThisMonth(new Date(tx.date)))
    );
          ...
          <div key={m.label} className={cn("p-4 space-y-3", m.isCurrentMonth && "bg-primary/5")}>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-success">{moneyUtils.format(m.income, "BRL")}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground flex items-center gap-1">
                ...
                <span className="text-xs font-semibold text-destructive">{moneyUtils.format(m.expense, "BRL")}</span>
              </div>
              <div className="pt-1.5 border-t border-border/40 flex items-center justify-between">
                ...
                <span className={cn("text-xs font-bold", m.net >= 0 ? "text-success" : "text-destructive")}>
                  {m.net >= 0 ? "+" : ""}{moneyUtils.format(m.net, "BRL")}
                </span>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-border/40">
              ...
              <p className={cn("text-sm font-bold mt-0.5", m.runningBalance >= 0 ? "text-foreground" : "text-destructive")}>
