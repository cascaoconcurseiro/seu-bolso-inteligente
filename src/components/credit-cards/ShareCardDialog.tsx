...
import {
  useSharedCreditCards,
  useInviteSharedCard,
  useRevokeSharedCard,
} from "@/hooks/useSharedCreditCards";
...
export function ShareCardDialog({ isOpen, onClose, card }: ShareCardDialogProps) {
  ...
  const availableMembers = familyMembers.filter(
    (m) => m.linked_user_id && m.status === "active" && m.linked_user_id !== user?.id
  );
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-full !bottom-0 !top-auto !translate-y-0 sm:!top-[50%] sm:!bottom-auto sm:!-translate-y-1/2 rounded-t-[2rem] sm:!rounded-4xl !rounded-b-none sm:!rounded-b-[2rem] p-0 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] sm:shadow-lg max-h-[90vh] flex flex-col border-b-0 sm:border-b bg-background pb-[env(safe-area-inset-bottom)] overflow-hidden pb-[env(safe-area-inset-bottom)]">
        <DialogHeader className="px-5 pt-5 pb-2 sm:px-6 sm:pt-6">
          ...
          <DialogDescription>
            Convide membros da sua família para compartilhar o limite e a fatura do cartão{" "}
            <b>{card.name}</b>. As compras de cada um serão isoladas para maior privacidade.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 px-5 pb-5 sm:px-6 sm:pb-6 overflow-y-auto flex-1">
          <div className="bg-warning/8 p-3 flex gap-3 rounded-lg border border-warning/20">
            ...
            <p className="text-sm text-warning">
              O convidado não poderá editar o Vencimento nem o Fechamento da fatura. Ao fechar a
              fatura, ele fará o acerto do valor devido para você.
            </p>
          </div>
            ...
            {membersLoading ? (
              ...
              <div className="space-y-2">
                {availableMembers.map((member) => {
                  const isAlreadyShared = sharedCards.some(
                    (sc) => sc.user_id === member.linked_user_id
                  );
                  ...
                  return (
                    <div
                      key={member.id}
                      className="flex items-center justify-between p-2 border border-transparent hover:border-border rounded-lg transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <UserAvatar
                          name={member.name}
      ...
      <Dialog
        open={!!selectedMemberForInvite}
        onOpenChange={(o) => !o && setSelectedMemberForInvite(null)}
      >
