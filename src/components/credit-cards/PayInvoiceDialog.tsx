...
export function PayInvoiceDialog({ isOpen, onClose, card, invoiceTotal, accounts, onPay }: PayInvoiceDialogProps) {
  ...
  // Sincronizar amountToPay quando invoiceTotal muda (ex: carregamento inicial)
  React.useEffect(() => {
    if (invoiceTotal > 0) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {isInternationalCard 
              ? `Fatura em ${cardCurrency} - selecione o valor e a conta de origem`
              : `Selecione o valor e a conta de origem para pagar`
            }
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          >
            ...
            Pagar {currentAmountToPay >= invoiceTotal ? 'Fatura Total' : 'Valor Parcial'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
