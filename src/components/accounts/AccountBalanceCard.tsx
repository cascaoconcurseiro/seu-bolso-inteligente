...
export function AccountBalanceCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl" style={{ backgroundColor: bgColor }}>
      {/* Decoração de fundo */}
      <div
      ...
      <div className="relative p-6 md:p-8">
        ...
        <p
          className={cn(
            "font-mono text-4xl md:text-5xl font-bold mb-2",
        ...
        {isCredit && account.credit_limit && (
          <p
            className={cn("text-sm mb-1", shouldHideBalance && "blur-md opacity-50 select-none")}
            style={{ color: textColor, opacity: 0.75 }}
          >
            Limite disponível:{" "}
            {shouldHideBalance
              ? "•••••"
              : formatCurrency(Number(account.credit_limit), accountCurrency)}
          </p>
        )}
        ...
        {/* Ações */}
        <div
          className="flex flex-wrap items-center gap-2 mt-6 pt-5"
        ...
        >
          ...
          {canDelete && (
            <Button
              ...
              variant="secondary"
              size="sm"
              className="gap-1.5 bg-red-500/30 hover:bg-red-500/50 text-white border-0 backdrop-blur-sm"
            >
