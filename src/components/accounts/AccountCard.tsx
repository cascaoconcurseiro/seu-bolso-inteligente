  ...
  return (
    <div className="group flex flex-col rounded-2xl border border-border/50 bg-card hover:border-primary/30 transition-all duration-300 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 overflow-hidden focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2 focus-within:ring-offset-background">
      <Link
        ...
        aria-label={`Conta: ${account.name}, saldo ${formatCurrency(account.balance)}`}
      >
        {/* Card Header visual mimicry */}
        <div
          className="p-4 md:p-5 relative overflow-hidden"
          style={{ backgroundColor: account.bank_color || bank.color }}
        >
          {/* Subtle glowing orb */}
          ...
          <div className="flex items-center justify-between gap-3 relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p
                  className="text-sm uppercase font-bold tracking-widest opacity-80"
                  style={{ color: bank.textColor }}
                >
                  {isInternational
                    ? "Conta Global"
            ...
            {isInternational && (
              <span
                className="text-sm bg-white/20 dark:bg-black/20 px-2 py-0.5 rounded-full font-bold flex items-center gap-2 backdrop-blur-sm"
                style={{ color: bank.textColor }}
              >
          ...
          <div className="mt-4 md:mt-6 relative z-10">
            <p
              className="text-sm uppercase tracking-wider font-bold opacity-75"
              style={{ color: bank.textColor }}
            >
              Saldo Disponível
            </p>
            <p
              className={cn(
                "font-mono text-2xl sm:text-3xl font-black tracking-tight mt-1",
                shouldHideBalance && "blur-md opacity-50 select-none"
              )}
              style={{ color: bank.textColor }}
            >
              {shouldHideBalance
            ...
            <div className="space-y-2.5">
              {lastTransactions.map((tx) => {
                ...
                const displayAmt =
                  ...
                  tx.destination_amount !== undefined
                ...
                return (
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      window.dispatchEvent(
                        new CustomEvent("openTransactionModal", { detail: { transaction: tx } })
                      );
                    }}
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        "font-mono font-bold ml-2 shrink-0 tabular-nums",
                        isIncome ? "text-positive" : "text-negative"
                      )}
                    >
                      {isIncome ? "+" : "-"}
                      {isInternational ? currencySymbol : ""}
                      {isInternational ? displayAmt.toFixed(2) : formatCurrency(displayAmt)}
                    </span>
                  </div>
                );
              })}
