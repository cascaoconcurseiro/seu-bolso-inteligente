...
const DIVIDEND_TYPES_BY_ASSET: Record<string, { value: string; label: string }[]> = {
  ...
  ETF: [{ value: "DIVIDENDOS", label: "Dividendos" }],
  REIT: [{ value: "DIVIDENDOS", label: "Dividendos" }],
  FUND: [
    { value: "RENDIMENTO", label: "Rendimentos" },
    { value: "AMORTIZACAO", label: "Amortização" },
  ],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (type === "buy") {
        ...
        const dividendLabel =
          (DIVIDEND_TYPES_BY_ASSET[asset.type] || DIVIDEND_TYPES_BY_ASSET["DEFAULT"]).find(
            (d) => d.value === dividendType
          )?.label || "Rendimento";
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && resetAndClose()}>
      <DialogContent className="sm:max-w-[440px] w-full !bottom-0 !top-auto !translate-y-0 sm:!top-[50%] sm:!bottom-auto sm:!-translate-y-1/2 rounded-t-[2rem] sm:!rounded-4xl !rounded-b-none sm:!rounded-b-[2rem] p-0 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] sm:shadow-lg max-h-[90vh] flex flex-col border-b-0 sm:border-b bg-background pb-[env(safe-area-inset-bottom)] overflow-hidden pb-[env(safe-area-inset-bottom)]">
        <DialogHeader className="px-5 pt-5 pb-2 sm:px-6 sm:pt-6">
          ...
          <DialogDescription>
            PM atual:{" "}
            <span className="font-mono font-semibold">{formatMoney(currentAvgPrice)}</span>
        ...
        <form
          ...
          className="space-y-5 mt-2 px-5 pb-5 sm:px-6 sm:pb-6 overflow-y-auto flex-1"
        >
          {/* Tabs de operação */}
          ...
          {type !== "update" && (
            <Alert className="border-warning/20 bg-warning/8">
              ...
              <AlertDescription className="text-sm text-warning">
                ...
                {type === "dividend" &&
                  "O valor do rendimento será creditado como receita na conta selecionada."}
              </AlertDescription>
            </Alert>
          )}
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="space-y-2">
                ...
                <Select value={dividendType} onValueChange={setDividendType}>
                  ...
                  <SelectContent>
                    {(
                      DIVIDEND_TYPES_BY_ASSET[asset.type] || DIVIDEND_TYPES_BY_ASSET["DEFAULT"]
                    ).map((opt) => (
              ...
              {type === "sell" && sellPnl !== null && (
                <div className="flex justify-between border-t border-border pt-2 mt-2">
                  ...
                  <span
                    className={cn(
                      "font-mono font-bold",
                      sellPnl >= 0 ? "text-success" : "text-destructive"
                    )}
                  >
                    {sellPnl >= 0 ? "+" : ""}
                    {formatMoney(sellPnl)}
            ...
            <div className="space-y-2">
              ...
              {isAbroad && selectedAccount && !selectedAccount.is_international && (
                <p className="text-sm text-warning flex items-center gap-1">
                  ...
                  Conta em BRL selecionada. Considere usar uma conta internacional para ativos em{" "}
                  {currency}.
                </p>
              )}
            </div>
