        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5 sm:px-6 sm:pb-6 mt-2 space-y-3">
          {isLoading ? (
            ...
            history?.map((tx) => (
              <div
                key={tx.id}
                className="p-4 rounded-xl border border-border bg-card/50 flex items-center justify-between group hover:bg-card transition-colors"
              >
                <div className="flex items-center gap-3">
                  <div
                    className={cn(
                      "w-10 h-10 rounded-full flex items-center justify-center",
                      tx.type === "BUY" && "bg-success/12 text-success",
                      tx.type === "SELL" && "bg-destructive/12 text-destructive",
                  ...
                  {tx.type.startsWith("DIVIDEND") ? (
                    ...
                    <>
                      <p className="text-sm font-mono font-bold text-foreground">
                        {tx.quantity}{" "}
                        <span className="text-sm text-muted-foreground font-sans">unid.</span>
                      </p>
