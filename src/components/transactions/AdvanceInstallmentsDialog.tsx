...
export function AdvanceInstallmentsDialog({
  ...
  const formatCompetence = (dateStr: string) => {
    ...
    return dateFns.format(date, "MMMM 'de' yyyy", { locale: ptBR });
  };
  ...
  const totalToAdvance = futureInstallments
    ...
    .reduce((sum, i) => SafeFinancialCalculator.add(sum, Number(i.amount)).toNumber(), 0);

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md w-full !bottom-0 !top-auto !translate-y-0 sm:!top-[50%] sm:!bottom-auto sm:!-translate-y-1/2 rounded-t-[2rem] sm:!rounded-4xl !rounded-b-none sm:!rounded-b-[2rem] p-0 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] sm:shadow-lg max-h-[90vh] flex flex-col border-b-0 sm:border-b bg-background pb-[env(safe-area-inset-bottom)] overflow-hidden pb-[env(safe-area-inset-bottom)]">
        <DialogHeader>
