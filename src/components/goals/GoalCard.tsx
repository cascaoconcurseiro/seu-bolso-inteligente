...
export function GoalCard({ goal, index, onEdit, onDelete, onContribute }: GoalCardProps) {
  ...
  const estimatedCompletionDate =
    estimatedMonthsLeft !== null ? addMonths(new Date(), estimatedMonthsLeft) : null;
      ...
      >
        ...
        <div className="relative">
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="hidden sm:flex gap-1 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                <Button
                  variant="ghost"
                  size="icon"
                  aria-label="Exportar meta em PDF"
                  onClick={async (e) => {
                    e.stopPropagation();
                    try {
                      await exportGoalToPDF(goal);
                    } catch {
          ...
          <div className="space-y-4">
                    ...
                    <Tooltip delayDuration={300}>
                      <TooltipTrigger asChild>
                        <p className="text-sm font-mono font-bold text-warning/80 mt-0.5 flex items-center justify-end gap-1 cursor-help hover:text-warning transition-colors">
                          Real: {moneyUtils.format(adjustedTarget, "BRL")}{" "}
                          <Info className="h-3 w-3" />
                        </p>
                      </TooltipTrigger>
                      <TooltipContent className="w-64 p-3 bg-card border-border/50 shadow-xl rounded-xl z-50">
                        <p className="text-sm leading-relaxed text-foreground/90">
                          Devido à inflação atual de{" "}
                          <strong className="text-warning">
                            {indicators?.ipca?.value}% ao ano
                          </strong>{" "}
                          (IPCA BCB), você precisará de{" "}
                          <strong className="font-mono">
                            {moneyUtils.format(adjustedTarget, "BRL")}
                          </strong>{" "}
                          em {monthsToTarget} meses para ter o mesmo poder de compra de{" "}
                          {moneyUtils.format(goal.target_amount, "BRL")} hoje.
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                ...
                {remaining > 0 && (
                  <p className="text-sm font-medium text-muted-foreground">
                    ...
                    <span className="text-foreground font-bold">
                      {moneyUtils.format(remaining, "BRL")}
                    </span>
                  </p>
                )}
              </div>
                ...
                {monthlyNeeded !== null && monthlyNeeded > 0 && (
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-bold text-primary">
                      {moneyUtils.format(monthlyNeeded, "BRL")}
                    </span>
                  </div>
                )}
            ...
            >
              <ChevronDown
                className={cn("h-3.5 w-3.5 transition-transform", showMilestones && "rotate-180")}
              />
