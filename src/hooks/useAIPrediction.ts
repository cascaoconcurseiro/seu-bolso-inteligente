...
export function useAIPrediction(
  ...
  useEffect(() => {
    ...
    if (
      description.trim() === lastPredictedDescRef.current &&
      categoriesHash === lastHashRef.current
    ) {
      return;
    }
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const historyTransactions = (transactionsRef.current || []).filter(
          (t) =>
            t.type === targetType &&
            t.description &&
        ...
        for (const t of historyTransactions) {
          ...
          if (cat) {
            const parentCat = cat.parent_category_id
              ? allCatsForHistory.find((c) => c.id === cat.parent_category_id)
              : null;
        ...
        const filteredCats = allCats.filter((c) => {
          ...
          const allowedParents =
            type === "expense" ? ALLOWED_EXPENSE_PARENTS : ALLOWED_INCOME_PARENTS;
        ...
        if (currentRequestId !== requestCounterRef.current) {
          logger.debug(
            `[useAIPrediction] [Req #${currentRequestId}] Descartando resultado obsoleto — req #${requestCounterRef.current} mais recente`
          );
          return;
        }
        ...
        if (catId && !useSubcategories && categoriesRef.current) {
          ...
          if (found && found.parent_category_id) {
            logger.debug(
              `[useAIPrediction] Mapeando subcategoria "${found.name}" para categoria pai`
            );
        ...
        if (currentRequestId === requestCounterRef.current) {
          logger.error(
            `[useAIPrediction] [Req #${currentRequestId}] Erro na chamada de IA`,
