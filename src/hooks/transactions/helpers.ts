...
export async function validatePayerId(payerId: string | null | undefined): Promise<boolean> {
  ...
  try {
    ...
    if (payerCheckError) {
      logger.error("Erro ao validar payer_id em family_members:", {
        payer_id: payerId,
        error: payerCheckError,
      });
    ...
    if (profileCheckError) {
      logger.error("Erro ao validar payer_id em profiles:", {
        payer_id: payerId,
    ...
    if (
      ...
      (error.message.includes("O pagador selecionado") || error.message.includes("Erro ao validar"))
    ) {
      throw error;
    }
...
export async function validateMemberId(memberId: string | null | undefined): Promise<boolean> {
  ...
  try {
    ...
    if (memberCheckError) {
      logger.error("Erro ao validar member_id em family_members:", {
        member_id: memberId,
    ...
    if (profileCheckError) {
      logger.error("Erro ao validar member_id em profiles:", {
        member_id: memberId,
        error: profileCheckError,
      });
      throw new Error("Erro ao validar membro. Tente novamente.");
    ...
    if (
      error instanceof Error &&
      (error.message.includes("O membro selecionado") || error.message.includes("Erro ao validar"))
    ) {
      throw error;
    }
