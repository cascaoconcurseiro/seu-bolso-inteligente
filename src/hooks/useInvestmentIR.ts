...
export function useInvestmentIR({ assets }: InvestmentIRPanelProps) {
  ...
  const cnpjMap: Record<string, string> = {
    ...
    ITSA4: "17.217.989/0001-04",
    ITSA3: "17.217.989/0001-04",
    MXRF11: "12.006.126/0001-60",
    HGLG11: "11.728.847/0001-70",
    XPML11: "28.757.540/0001-48",
    KNIP11: "23.223.940/0001-38",
    XPLG11: "26.685.992/0001-24",
    KNRI11: "12.005.956/0001-65",
    HGRE11: "09.072.068/0001-16",
    VISC11: "18.860.759/0001-30",
  };
  ...
  incomeTxs.forEach((tx) => {
    ...
    if (
      catName === "Dividendos" ||
      desc.toLowerCase().includes("dividendo") ||
      desc.toLowerCase().includes("prov.")
    ) {
      const code = "09";
      ...
      isentosMap[key].valor = SafeFinancialCalculator.add(isentosMap[key].valor, amount).toNumber();
      isentosMap[key].detalhes.push(
        `${tx.date.split("-").reverse().slice(0, 2).join("/")}: R$ ${amount.toFixed(2)} (${desc})`
      );
    } else if (
      catName === "Fundos Imobiliários" ||
      desc.toLowerCase().includes("fii") ||
      desc.toLowerCase().includes("rend. fii")
    ) {
      const code = "26";
      const key = `${code}-${keyPrefix}`;
      if (!isentosMap[key]) {
      ...
      catName === "Rendimento Poupança" ||
      desc.toLowerCase().includes("poupança") ||
      desc.toLowerCase().includes("lci") ||
      desc.toLowerCase().includes("lca")
    ) {
      const code = "12";
      const key = `${code}-${keyPrefix}`;
      if (!isentosMap[key]) {
        isentosMap[key] = {
          codigo:
            "12 - Rendimentos de cadernetas de poupança, letras de crédito (LCI, LCA, CRI, CRA, etc.)",
  ...
  const tributacaoExclusivaMap: Record<
    string,
    {
      codigo: string;
      tipo: string;
  ...
  incomeTxs.forEach((tx) => {
    ...
    if (
      catName === "Juros" ||
      desc.toLowerCase().includes("jcp") ||
      desc.toLowerCase().includes("juros sobre capital") ||
      desc.toLowerCase().includes("juros s/")
    ) {
      const code = "10";
      const key = `${code}-${keyPrefix}`;
      ...
      tributacaoExclusivaMap[key].valor = SafeFinancialCalculator.add(
        tributacaoExclusivaMap[key].valor,
        amount
      ).toNumber();
      tributacaoExclusivaMap[key].detalhes.push(
        `${tx.date.split("-").reverse().slice(0, 2).join("/")}: R$ ${amount.toFixed(2)} (${desc})`
      );
    } else if (
      catName === "Rendimento CDB" ||
      desc.toLowerCase().includes("cdb") ||
      desc.toLowerCase().includes("tesouro direto") ||
      desc.toLowerCase().includes("rdb")
    ) {
      const code = "06";
      const key = `${code}-${keyPrefix}`;
      if (!tributacaoExclusivaMap[key]) {
        tributacaoExclusivaMap[key] = {
          codigo:
            "06 - Rendimentos de aplicações financeiras (CDB, RDB, Tesouro Direto, Debêntures Comuns)",
  ...
  operationsOfYear.forEach((tx) => {
    ...
    if (tx.type === "BUY") {
      monthlyResumo[m].compras = SafeFinancialCalculator.add(
        monthlyResumo[m].compras,
        amount
      ).toNumber();
    } else if (tx.type === "SELL") {
      monthlyResumo[m].vendas = SafeFinancialCalculator.add(
        monthlyResumo[m].vendas,
        amount
      ).toNumber();
      ...
      if (asset) {
        ...
        priorTxs.forEach((t) => {
          ...
          if (t.type === "BUY") {
            totalCost = SafeFinancialCalculator.add(
              totalCost,
              SafeFinancialCalculator.multiply(tQty, tPrice)
            ).toNumber();
            qty = SafeFinancialCalculator.add(qty, tQty).toNumber();
          } else if (t.type === "SELL") {
            if (qty > 0) {
              const currentAvg = SafeFinancialCalculator.divide(totalCost, qty);
              const soldQty = Math.min(tQty, qty);
              qty = SafeFinancialCalculator.subtract(qty, soldQty).toNumber();
              totalCost = SafeFinancialCalculator.multiply(qty, currentAvg).toNumber();
            }
          }
        });
        ...
        const avgPrice =
          qty > 0
            ? SafeFinancialCalculator.divide(totalCost, qty).toNumber()
            : Number(asset.purchase_price || 0);
        const salePrice = Number(tx.price);
        const saleQty = Number(tx.quantity);
        const profit = SafeFinancialCalculator.multiply(
          SafeFinancialCalculator.subtract(salePrice, avgPrice),
          saleQty
        ).toNumber();

        monthlyResumo[m].lucroEstimado = SafeFinancialCalculator.add(
          monthlyResumo[m].lucroEstimado,
          profit
        ).toNumber();

        if (asset.type === "STOCK") {
          monthlyResumo[m].vendasAcoes = SafeFinancialCalculator.add(
            monthlyResumo[m].vendasAcoes,
            amount
          ).toNumber();
          if (profit > 0) {
            monthlyResumo[m].lucroVendaAcoes = SafeFinancialCalculator.add(
              monthlyResumo[m].lucroVendaAcoes,
              profit
            ).toNumber();
          } else {
            monthlyResumo[m].prejuizoAcumulado = SafeFinancialCalculator.add(
              monthlyResumo[m].prejuizoAcumulado,
              Math.abs(profit)
            ).toNumber(); // Salva como positivo
          }
        }
      }
    }
