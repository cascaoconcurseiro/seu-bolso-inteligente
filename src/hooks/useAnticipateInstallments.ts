  ...
  return useMutation({
    mutationFn: async (params: AnticipateInstallmentsParams) => {
      ...
      for (const installment of installments) {
        ...
        const hasSettledSplit = installmentSplits.some(
          (split) => split.is_settled || split.settled_by_debtor || split.settled_by_creditor
        );
      ...
      if (settledInstallments.length > 0) {
        logger.error(
          "❌ [useAnticipateInstallments] Parcelas acertadas encontradas:",
          settledInstallments
        );
      ...
      if (duplicateError) {
        logger.error(
          "❌ [useAnticipateInstallments] Erro ao verificar duplicatas:",
          duplicateError
        );
      ...
      if (existingInstallments && existingInstallments.length > 0) {
        logger.error(
          "❌ [useAnticipateInstallments] DUPLICAÇÃO DETECTADA - Já existe parcela com esta competence_date:",
          {
            seriesId,
            newCompetenceDate,
        ...
        throw new Error(
          `Já existe uma parcela com a data de competência ${newCompetenceDate}. Escolha outra data para evitar duplicação.`
        );
      }

      logger.debug(
        "✅ [useAnticipateInstallments] Validação de duplicação passou - nenhuma duplicata encontrada"
      );
