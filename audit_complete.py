...
def phase2_referential_integrity():
    ...
    for m in missing_idx:
        print(f"    ⚠️ {m['table_name']}.{m['column_name']} ({m['constraint_name']})")
...
def phase6_financial_calculations():
    ...
    for b in balance_check:
        print(f"      ❌ {b['name']}: stored={b['stored_balance']}, calculated={b['calculated_balance']}, diff={b['difference']}")
        issues.append({
    ...
    for inv in invoice_check:
        print(f"      ❌ Invoice {inv['invoice_id'][:8]}: total={inv['invoice_total']}, sum={inv['sum_transactions']}, diff={inv['diff']}")
        issues.append({
    ...
    for g in goal_check:
        print(f"      ❌ {g['name']}: current={g['current_amount']}, calc={g['calc_current']}, diff={g['diff']}")
    ...
    for b in budget_check:
        print(f"      ⚠️ Budget {b['id'][:8]}: stored_spent={b['budget_spent']}, actual={b['actual_spent']}")
    ...
    for tr in trip_check:
        print(f"      ⚠️ {tr['name']}: stored_spent={tr['stored_spent']}, calc={tr['calc_spent']}")
...
def phase15_sensitive_data():
    ...
    if has_plain:
        ...
        print(f"  ⚠️ Usuários com PIN plaintext: {plain_count[0]['count']}")
...
|------------|------------|
| 🔴 CRÍTICO | {scores['counts'].get('CRÍTICO', 0)} |
| 🟠 ALTO    | {scores['counts'].get('ALTO', 0)} |
| 🟡 MÉDIO   | {scores['counts'].get('MÉDIO', 0)} |
| 🔵 BAIXO   | {scores['counts'].get('BAIXO', 0)} |
...
"""
    ...
    for i, issue in enumerate(all_issues, 1):
        sev_emoji = {"CRÍTICO": "🔴", "ALTO": "🟠", "MÉDIO": "🟡", "BAIXO": "🔵"}.get(issue.get("severity", ""), "⚪")
