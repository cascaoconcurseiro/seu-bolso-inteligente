-- Fix dashboard recent activity sort order: use created_at DESC instead of date DESC.
-- This ensures the most recently created transactions appear first, regardless of
-- the transaction date (which can be in the future for installments).
...
BEGIN
    ...
    recent AS (
        SELECT t.id, t.type, t.amount, COALESCE(t.currency, 'BRL') as currency, t.date, t.description,
               t.is_shared, t.payer_id, t.account_id, t.source_transaction_id, t.creator_user_id,
        ...
        WHERE t.deleted_at IS NULL
          ...
          AND (
            (v_member_id IS NOT NULL AND (t.user_id = p_user_id OR t.payer_id = v_member_id))
            OR
            (v_member_id IS NULL AND t.user_id = p_user_id)
          )
