-- ============================================================================
-- Migration: Dashboard uses cash-basis for credit card expenses
-- Purpose:
-- Credit card individual charges should NOT appear in Entradas/Saídas summary.
-- Only the actual invoice payment from checking account counts as a "saída".
-- This prevents double-counting: charge in month X + invoice paid in month Y.
-- ============================================================================
...
BEGIN
    ...
    WITH tx_raw AS (
        ...
        WHERE t.user_id = p_user_id
          ...
          AND t.type != 'TRANSFER'
          -- Regime de caixa: excluir despesas de cartão de crédito.
          -- Elas são contabilizadas quando a fatura é paga (saída da conta corrente).
