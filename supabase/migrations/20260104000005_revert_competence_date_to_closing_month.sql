-- Reverter competence_date para usar mês de FECHAMENTO (não vencimento)
-- competence_date = mês em que a fatura FECHA
-- O mês de vencimento será calculado dinamicamente nos Compartilhados

-- Recriar função com lógica de fechamento
...
BEGIN
  RAISE NOTICE 'Revertendo competence_date para mês de fechamento...';
...
-- Remover função de vencimento (não é mais necessária)
DROP FUNCTION IF EXISTS public.calculate_shared_transaction_competence_date(DATE, INTEGER, INTEGER);
