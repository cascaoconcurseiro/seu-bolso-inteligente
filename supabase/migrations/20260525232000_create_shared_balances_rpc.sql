-- Migration para transferir o cálculo de saldos compartilhados do Frontend para o Banco de Dados

CREATE OR REPLACE FUNCTION get_current_shared_debts(p_user_id UUID)
RETURNS TABLE (
    member_id UUID,
    currency TEXT,
...
BEGIN
    ...
    -- Caso 2B: Paid by others sem split (Direto payer_id)
    debits_direct AS (
        SELECT 
    ...
    -- Caso 3: Atribuição Direta (related_member_id)
    credits_related AS (
        SELECT 
