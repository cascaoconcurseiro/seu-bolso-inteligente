-- Corrigir função create_transaction_mirrors (remover coluna category)
...
BEGIN
  ...
  -- Usar linked_user_id (pessoa real)
  ...
  AND linked_user_id = original_transaction.user_id
  LIMIT 1;
  ...
  INSERT INTO transactions (
    ...
    NEW.amount,
    original_transaction.description || ' (Compartilhado por ' || COALESCE((SELECT name FROM family_members WHERE id = payer_member_id LIMIT 1), 'Outro') || ')',
