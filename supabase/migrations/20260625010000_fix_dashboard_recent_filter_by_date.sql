-- Fix: recent_transactions filtra por t.date (igual à página de transações),
-- não por competence_date. Assim parcelas com competence em outro mês não aparecem.
...
BEGIN
    ...
    tx AS (
        SELECT type, amount, is_refund, source_transaction_id, currency, ref_date, date
        FROM tx_raw
        WHERE ref_date >= p_start_date AND ref_date <= p_end_date
    ),
    -- Atividade recente: filtra por t.date (igual à página de transações),
    -- sem month-lock por competence_date para evitar parcelas de outros meses.
    recent AS (
        SELECT t.id, t.type, t.amount, COALESCE(t.currency, 'BRL') as currency,
