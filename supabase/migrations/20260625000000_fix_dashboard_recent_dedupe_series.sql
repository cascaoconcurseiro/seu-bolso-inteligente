-- Fix: deduplica parcelas na atividade recente do dashboard.
-- Antes: todas as parcelas de uma série com competence_date no período apareciam.
-- Agora: por series_id, mostra apenas a parcela mais recente (maior competence_date).
...
BEGIN
    ...
    -- Para cada série (series_id), seleciona apenas a parcela mais recente dentro do período.
    -- Transações sem série são tratadas individualmente (series_id IS NULL → usa o próprio id).
    recent_deduped AS (
        SELECT DISTINCT ON (COALESCE(t.series_id::text, t.id::text))
               t.id, t.type, t.amount, COALESCE(t.currency, 'BRL') as currency,
               t.date, t.description, t.is_shared, t.payer_id, t.account_id,
               t.source_transaction_id,
               COALESCE(t.competence_date, t.date) AS sort_date,
               t.created_at,
        ...
        ORDER BY COALESCE(t.series_id::text, t.id::text),
                 COALESCE(t.competence_date, t.date) DESC,
                 t.created_at DESC
    ),
    recent AS (
        SELECT id, type, amount, currency, date, description, is_shared, payer_id,
               account_id, source_transaction_id, category, account
        FROM recent_deduped
