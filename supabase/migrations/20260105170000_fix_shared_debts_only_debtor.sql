-- Fix: Corrigir cálculo de shared_debts para mostrar apenas dívidas do usuário
-- Problema: Está somando TODOS os splits do usuário, incluindo quando ele é credor
-- Solução: Filtrar apenas splits onde user_id != creator_user_id (ou seja, ele é devedor)
...
BEGIN
  ...
  -- 🔧 CORREÇÃO CRÍTICA: Compartilhados a pagar
  -- Só deve contar splits onde:
  -- 1. user_id = p_user_id (eu sou o devedor no split)
  -- 2. creator_user_id != p_user_id (eu NÃO criei a transação, ou seja, sou devedor)
  -- 3. is_settled = false (ainda não paguei)
  SELECT COALESCE(SUM(ts.amount), 0) INTO v_shared_debts
  FROM public.transaction_splits ts
  INNER JOIN public.transactions t ON t.id = ts.transaction_id
  WHERE ts.user_id = p_user_id
    AND t.creator_user_id != p_user_id  -- 🔧 FILTRO CRÍTICO: Não sou o criador
...
COMMENT ON FUNCTION public.get_monthly_projection IS 
  'Calcula projeção do fim do mês considerando saldo, receitas/despesas futuras, cartões e compartilhados pendentes (apenas dívidas)';
