-- Migração: Correção lógica matemática temporal para a Projeção do Fim do Mês
...
BEGIN
  ...
  -- =====================================================
  -- CORREÇÃO CRÍTICA:
  -- Se for o mês atual, a receita é "futura" se a competência for em Maio E a data de realização (date) for maior que hoje.
  -- Usar competence_date > v_today causava o erro de ignorar receitas cuja competência é normalizada no dia 1º.
  ...
  WHERE t.user_id = p_user_id
    ...
    AND (
      -- Se for mês atual: competência no mês E data física futura (ainda não impactou accounts.balance)
      (v_start_of_month <= v_today AND v_end_of_month >= v_today 
       ...
       AND t.date > v_today)
      OR
      -- Se for mês futuro: tudo do mês (toda competência do mês selecionado)
  ...
  -- =====================================================
  -- CORREÇÃO CRÍTICA:
  -- Se for o mês atual, a despesa é "futura" se a competência for em Maio E a data de realização (date) for maior que hoje.
  -- Usar competence_date > v_today causava o erro de ignorar despesas cuja competência é normalizada no dia 1º.
...
COMMENT ON FUNCTION public.get_monthly_projection IS 
  'Calcula a projeção financeira reativa e precisa temporalmente em qualquer moeda (p_currency) baseada no mês selecionado por p_end_date';
