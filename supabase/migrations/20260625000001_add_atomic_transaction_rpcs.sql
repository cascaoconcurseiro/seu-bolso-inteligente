-- ARC-01: Atomic single transaction + splits
-- ARC-02: Atomic installment series + splits
-- Prevents orphaned transactions when split insert fails

-- Single transaction with splits (atomic)
...
BEGIN
  ...
  INSERT INTO transactions (
    ...
    is_recurring, total_installments, current_installment, series_id, trip_id,
  ...
  IF jsonb_array_length(p_splits) > 0 THEN
    ...
    SELECT
      ...
      COALESCE(split->>'name', 'Membro'),
      COALESCE((split->>'is_settled')::boolean, false)
    FROM jsonb_array_elements(p_splits) AS split;
  END IF;

  SELECT to_jsonb(t) INTO v_result FROM transactions t WHERE id = v_tx_id;
  RETURN v_result;
END;
$$;

-- Installment series with splits per installment (atomic)
-- Each element of p_transactions may carry a 'splits' array
...
BEGIN
  ...
  LOOP
    ...
    v_splits := COALESCE(v_tx_record->'splits', '[]'::jsonb);
  ...
  RETURN jsonb_build_object(
    ...
    'count', array_length(v_tx_ids, 1)
  );
END;
$$;

GRANT EXECUTE ON FUNCTION create_transaction_with_splits(JSONB, JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION create_installment_series(JSONB) TO authenticated;
